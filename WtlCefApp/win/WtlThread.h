...
public:
		...
		: TBase(NULL, NULL)
	{
		...
		m_hThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) Proc, this, dwCreationFlags, &m_dwThreadId);
	}

	// @XNightSky
	static DWORD WINAPI Proc(
		LPVOID lpThreadParameter
		)
	{
		T * pWork = static_cast<T*>(lpThreadParameter);
		if (pWork)
			pWork->Run();
		return 0;
	}
...
// @XNightSky
template <class T, class TMessageLoop = CMessageLoop, class TBase = CGuiThread>
class CGuiThreadImpl2 :
	public CThreadImpl<T, TBase>,
	public CMessageFilter,
	public CMessageMap
{
...
public:
	CGuiThreadImpl2(CAppModule* pModule, DWORD dwCreationFlags = 0)
		: m_pModule(pModule), CThreadImpl<T, TBase>(dwCreationFlags)
	{
		ATLASSERT(pModule != NULL);
	}
