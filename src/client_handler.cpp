...
bool ClientHandler::OnConsoleMessage(CefRefPtr<CefBrowser> browser,
...

  {
    ...
    if (first_message) {
      std::stringstream ss;
	  ss << /*AppGetWorkingDirectory()*/XCefAppManage::GetWorkingDirectory().ToString();
#if defined(OS_WIN)
      ss << "\\";
#else
      ss << "/";
	...
	//if (file)
	FILE* file = NULL; errno_t err = fopen_s(&file, logFile.c_str(), "a");
	if (0 == err)
	{
    std::stringstream ss;
...
bool ClientHandler::Save(const std::string& path, const std::string& data) {
	...
	//if (!f)
	FILE* f = NULL; errno_t err = fopen_s(&f, path.c_str(), "w");
	if (0 != err)
		return false;
	size_t total = 0;
	do {
